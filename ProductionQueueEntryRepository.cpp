...
{
    ProductionQueueEntryRepository::ProductionQueueEntryRepository(std::filesystem::path jsonPath)
        : jsonPath_(std::move(jsonPath))
    {
    }

    void ProductionQueueEntryRepository::load()
    {
        ...
        entryList_ = json.get<std::vector<Model::ProductionQueueEntry>>();
    }

    void ProductionQueueEntryRepository::save() const
    {
        const nlohmann::json json = entryList_;
    ...
    const std::vector<Model::ProductionQueueEntry>& ProductionQueueEntryRepository::all() const
    {
        return entryList_;
    }
}
